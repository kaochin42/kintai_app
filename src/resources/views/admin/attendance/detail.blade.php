...
<div class="attendance-detail center">
    ...
    <form action="/attendance/detail/{{ $attendanceRecord->id }}" method="post" class="detail-table">
        ...
        <div class="detail-row">
            ...
            <input type="time" name="clock_in" value="{{ $attendanceRecord->clock_in }}">
            ～
            <input type="time" name="clock_out" value="{{ $attendanceRecord->clock_out }}">
        </div>

        @foreach($attendanceRecord->attendanceBreaks as $break)
        <div class="detail-row">
            <span class="detail-label">休憩{{ $loop->iteration }}</span>
            <input type="time" name="breaks[{{ $loop->index }}][break_in]" value="{{ $break->break_in }}"> ～
            <input type="time" name="breaks[{{ $loop->index }}][break_out]" value="{{ $break->break_out }}">
        </div>
        @endforeach
